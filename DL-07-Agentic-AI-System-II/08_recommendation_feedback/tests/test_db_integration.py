...
async def test_unsafe_feedback_appears_in_safety_queue(require_db):
    fb = FeedbackSubmission(
        request_id=ALL_SCENARIOS["avoid_travel"].request_id,
    ...
    await db.save_recommendation(ALL_SCENARIOS["avoid_travel"], pseudonymous_user_id="anon-test-2")
...
async def test_reviewed_feedback_moves_to_training_set(require_db):
    fb = FeedbackSubmission(
        request_id=ALL_SCENARIOS["change_route"].request_id,
        pseudonymous_user_id="anon-test-3",
        category=FeedbackCategory.INCORRECT,
        submitted_at=datetime.now(timezone.utc),
    )
    await db.save_recommendation(ALL_SCENARIOS["change_route"], pseudonymous_user_id="anon-test-3")
